 ...
 * - glowOpacity: number - Opacity of the polygons (0.5)
 * - glowHue: number - Hue shift for the polygons (default: 0)
 * - glowSeed: string | false - Seed for the stable random distribution (default: 4)
 */
...
const formatter = computed(() => {
  let result = (currentSlideRoute.value.meta?.slide as any)?.frontmatter || {};
  return result;
})

const slideContext = useSlideContext();
const meta = slideContext.$slidev.nav.slides[0].meta;
...
function usePloy(number = 16) {
  ...
  function jumpPoints() {
    ...
    points.value = points.value.map((o) => {
      ...
      return o // Fallback to the original point if no closest point is found
    })
  }
...
<template>
  <div v-if="glowEnabled" class="bg transform-gpu overflow-hidden pointer-events-none"
    :style="{ filter: `blur(70px) hue-rotate(${hue}deg)` }" aria-hidden="true">
    <div class="clip bg-gradient-to-r from-[--slidev-theme-primary] to-white"
      :style="{ 'clip-path': `polygon(${poly1})`, 'opacity': opacity }" />
    <div class="clip bg-gradient-to-l from-[--slidev-theme-primary-accent] to-white"
      :style="{ 'clip-path': `polygon(${poly2})`, 'opacity': opacity }" />
    <div class="clip bg-gradient-to-t from-[--slidev-theme-secondary] to-white"
      :style="{ 'clip-path': `polygon(${poly3})`, 'opacity': opacity / 2 }" />
  </div>
</template>
